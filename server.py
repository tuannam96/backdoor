...
def Main():
    ...
    try:
        s.bind((host, port))
        s.listen(5)
        resp(0, "Listening connection at {}:{}".format(host,port))
    except PermissionError:
        resp(1, "Permision Denied!")
        sys.exit()
    except OSError as e:
        if e.args[0] == 98:
            ...
            sys.exit()
    conn, addr = s.accept()
    resp(0, "4shell connected with:{}".format(str(addr[0])))
    try:
        ...
        if user == "root":
            ...
            print(f"[{Fore.BLUE}USER{Fore.RESET}) ] Shell is connected as user !")
            euid = 1
